...
void writeFlashByte(UINT8 inByte, UINT8* inAddressPtr) {
	...
	/*
	 * We need to setup to call the assembly routines.
	 * 
	 * The C-calling convention for HCS08 will put inByte on the stack and inAddressPtr in H:X.
	 * The assembly just needs to get inByte from the stack into A. (PULA)
	 */
	
	asm {
					pula	
					jsr		FlashProg1		;Jump to the ASM routine that does the actual work.
					jmp		End				;Jump to the end of the ASM code.
	...
	/*
	 * Now we resume the C-portion of the routine.
	 */
}